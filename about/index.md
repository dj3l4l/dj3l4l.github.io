...
Thankfully, such a daunting task is not one that we embark on from scratch. For the past hundred years or so, Bayesian probability theory has been used as an objective means to express subjective knowledge in numerical form. It has been proven that those objective rules mirror the axioms of logic (that is, if a decision violates the rules of Bayesian inference, then it must also violate logic).
